...
double generic_sort (int SORT_BYTES, int numIterations, size_t n, void *d_array0, cudaEvent_t &start, cudaEvent_t &stop)
{
...
int main (int argc, char **argv)
{
    ...
    void* d_array;
    checkCudaErrors( cudaMalloc(&d_array, 2*BUFSIZE*sizeof(uint64_t)));

    cudaEvent_t start, stop;
    checkCudaErrors( cudaEventCreate(&start));
    checkCudaErrors( cudaEventCreate(&stop));

    double numIterations = 1,  numElements = BUFSIZE;

    auto print = [&] (int bytes, int keysize, double totalTime) {
        totalTime /= (1000 * numIterations);

        printf("%d of %d: Throughput = %.4lf MElements/s, Time = %.5lf s\n",
               bytes, keysize, 1.0e-6f * numElements / totalTime, totalTime);
    };

    printf("Sorting %.0lf elements:\n", numElements);
    for(int i=1;i<=4;i++)  print (i, 4, generic_sort <uint32_t> (i, 100/i, BUFSIZE, d_array, start, stop));  printf("\n");
    for(int i=1;i<=8;i++)  print (i, 8, generic_sort <uint64_t> (i, 100/i, BUFSIZE, d_array, start, stop));
}
